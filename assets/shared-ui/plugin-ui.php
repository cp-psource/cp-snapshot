...
if ( ! class_exists( 'WDEV_Plugin_Ui' ) ) {
	...
	class WDEV_Plugin_Ui {
		...
		static public function reset() {
			self::$i10n = array(
				'empty_search' => __( 'Nothing found', 'psource' ),
				'default_msg_ok' => __( 'Okay, we saved your changes!', 'psource' ),
				'default_msg_err' => __( 'Oops, we could not do this...', 'psource' ),
			);
		}
		...
		static public function enqueue_late() {
			 ...
			 */
			do_action( 'psource_plugin_ui_enqueued' );
		}
		...
		static public function output( $commands = array() ) {
			 ...
			 *      'message' => '...'  (required!)
			 */
			$notice = apply_filters( 'psource-admin-notice', false );
			if ( $notice ) {
				$command = 'WDP';
				...
				<div class="frash-notice-logo"><span></span></div>
					...
					<div class="frash-notice-cta">
						...
						<button class="frash-notice-dismiss" data-msg="<?php echo esc_attr( $msg_dismiss ); ?>">
							<?php esc_html_e( 'Dismiss', 'psource' ); ?>
						</button>
						<?php endif; ?>
					</div>
					<?php endif; ?>
