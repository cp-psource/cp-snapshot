...
if ( ! class_exists( 'PSOURCESnapshot' ) ) {

	class PSOURCESnapshot {
		...
		private function __construct() {
			...
			$this->DEBUG = false;
			$this->_settings['SNAPSHOT_VERSION'] = '3.1.5';
			...
			$this->_new_ui_tester = new PSOURCESnapshot_New_Ui_Tester();
							...
							switch ( $action ) {
								case 'resend':
									...
									if ( $item['destination-sync'] == "mirror" ) {
										$snapshot_sync_files_option = 'psource_snapshot_sync_files_' . $item['timestamp'];
		...
		function snapshot_item_run_immediate( $item_key ) {
			wp_remote_post( get_option( 'siteurl' ) . '/wp-cron.php',
				array(
					...
					'sslverify' => false,
					'body' => array(
						'nonce' => wp_create_nonce( 'PSOURCESnapshot' ),
						'type' => 'start',
					),
					'user-agent' => 'PSOURCESnapshot',
				)
			);
...
//				echo "data_item<pre>"; print_r($data_item); echo "</pre>";
				...
				$snapshot_sync_files_option = 'psource_snapshot_sync_files_' . $item_key;
