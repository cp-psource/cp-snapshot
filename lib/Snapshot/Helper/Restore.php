...
class Snapshot_Helper_Restore {
	...
	private function _get_root () {
		return trailingslashit(wp_normalize_path(
			trailingslashit(wp_normalize_path(PSOURCESnapshot::instance()->get_setting('backupRestoreFolderFull'))) . '_imports'
		));
	}
	...
	private function _spawn_queues () {
		...
		// Boot session
		$loc = trailingslashit(PSOURCESnapshot::instance()->get_setting('backupSessionFolderFull'));
