...
if ( ! class_exists( 'Snapshot_Helper_Utility' ) ) {
	...
	class Snapshot_Helper_Utility {
				...
				if ( $tables_all_rows ) {
					foreach ( $wpdb->last_result as $table_set ) {
						foreach ( $table_set as $table_name ) {
							...
							if ( ( ! $table_structure ) || ( $table_structure == 0 ) ) {
								continue;
							}
		...
		public static function consume_archive_manifest( $manifest_file ) {
			...
			$sessionRestoreFolder = trailingslashit( PSOURCESnapshot::instance()->get_setting( 'backupRestoreFolderFull' ) );
		...
		public static function extract_archive_manifest( $archiveFilename = '', $restoreFolder = '' ) {
			...
			if ( ! $restoreFolder ) {
				$restoreFolder = PSOURCESnapshot::instance()->get_setting( 'backupRestoreFolderFull' );
			}
			...
			//echo "zipLibrary[". PSOURCESnapshot::instance()->config_data['config']['zipLibrary'] ."]<br />";
			//die();
		...
		public static function archives_import_proc( $restoreFile, $restoreFolder ) {
			...
			PSOURCESnapshot::instance()->load_config();
			PSOURCESnapshot::instance()->set_backup_folder();
			PSOURCESnapshot::instance()->set_log_folders();

			$CONFIG_CHANGED = false;

//	echo "restoreFile=[". $restoreFile ."]<br />";
//	echo "restoreFolder=[". $restoreFolder ."]<br />";
//	echo "before items<pre>"; print_r(PSOURCESnapshot::instance()->config_data['items']); echo "</pre>";
			...
			if ( file_exists( $snapshot_manifest ) ) {
				...
				if ( ! isset( PSOURCESnapshot::instance()->config_data['items'][ $item_key ] ) ) {
					PSOURCESnapshot::instance()->config_data['items'][ $item_key ] = $item;
					$CONFIG_CHANGED                                                = true;
					...
					foreach ( $item['data'] as $data_item_key => $data_item ) {
						...
						if ( ! isset( PSOURCESnapshot::instance()->config_data['items'][ $item_key ]['data'][ $data_item_key ] ) ) {
							PSOURCESnapshot::instance()->config_data['items'][ $item_key ]['data'][ $data_item_key ] = $data_item;
							$CONFIG_CHANGED                                                                          = true;
				...
				if ( $CONFIG_CHANGED == true ) {
					PSOURCESnapshot::instance()->save_config();
				}
			} else {
				$error_status['errorStatus'] = true;
				$error_status['errorText']   = __( "Manifest data not found in archive.", SNAPSHOT_I18N_DOMAIN );
		...
		public static function item_get_lock_info( $item_key = '' ) {
			...
			$lock_info['file']   = trailingslashit( PSOURCESnapshot::instance()->get_setting( 'backupLockFolderFull' ) ) . $item_key . ".lock";
