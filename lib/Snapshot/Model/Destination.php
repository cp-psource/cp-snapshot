...
abstract class Snapshot_Model_Destination {
	...
	public static function load_destinations() {
		...
		$dir = PSOURCESnapshot::instance()->get_plugin_path() . 'lib/Snapshot/Model/Destination';

		if ( ! defined( 'PSOURCE_SNAPSHOT_DESTINATIONS_EXCLUDE' ) ) {
			define( 'PSOURCE_SNAPSHOT_DESTINATIONS_EXCLUDE', '' );
		}
	...
	public static function show_destination_item_count( $destination_key ) {
		if ( isset( PSOURCESnapshot::instance()->config_data['items'] ) ) {
			$destination_count = 0;
			foreach ( PSOURCESnapshot::instance()->config_data['items'] as $snapshot_item ) {
