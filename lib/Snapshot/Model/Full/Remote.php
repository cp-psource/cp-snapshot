...
class Snapshot_Model_Full_Remote extends Snapshot_Model_Full_Abstract {
	...
	public function get_backup ($timestamp) {
		$path = trailingslashit(wp_normalize_path(PSOURCESnapshot::instance()->get_setting('backupRestoreFolderFull')));
	...
	public function continue_item_upload ($timestamp) {
		...
		$local_path = trailingslashit(wp_normalize_path(PSOURCESnapshot::instance()->get_setting('backupBaseFolderFull')));
	...
	private function _delete_backup ($timestamp) {
		...
		$local_path = trailingslashit(wp_normalize_path(PSOURCESnapshot::instance()->get_setting('backupRestoreFolderFull')));
		$status = false;
