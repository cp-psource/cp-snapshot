...
class Snapshot_Model_Full_Remote_Api extends Snapshot_Model_Full {
	...
	public function connect () {
		...
		if (empty($body)) {
			...
			$error = sprintf(
				...
				sprintf(Snapshot_View_Full_Backup::get_message('check_connection'), $this->get_dev_remote_host()),
				sprintf(Snapshot_View_Full_Backup::get_message('open_ticket'), 'https://premium.psource.org/forums/tags/snapshot-pro')
			);
	...
	public function get_dev_remote_host () {
		return defined('PSOURCE_CUSTOM_API_SERVER') && PSOURCE_CUSTOM_API_SERVER
			? PSOURCE_CUSTOM_API_SERVER
			: 'https://premium.psource.org/'
		;
	}
	...
	public function get_dashboard_api_key () {
		return apply_filters(
			...
			get_site_option('psource_apikey', false)
		);
	}
}