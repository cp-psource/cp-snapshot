...
class Snapshot_Model_Manifest {
	...
	protected function _get_segment_size () {
		return (int)PSOURCESnapshot::instance()->config_data['config']['segmentSize'];
	}

	protected function _get_queues () {
		if (!empty($this->_backup) && is_callable(array($this->_backup, 'get_queues'))) return $this->_backup->get_queues();
		return array();
	}
