	...
	class Snapshot_View_Destination_Listing {
				...
				<div id="snapshot-edit-destinations-panel" class="wrap snapshot-wrap">
					...
					if ( ( $destinationClasses ) && ( count( $destinationClasses ) ) ) {
						...
						<div class="snapshot-destination">
							...
							echo PSOURCESnapshot::instance()->get_setting( 'SNAPSHOT_MENU_URL' ); ?>snapshots_destinations_panel"
							      method="post">
								...
								<?php
								$edit_url   = PSOURCESnapshot::instance()->get_setting( 'SNAPSHOT_MENU_URL' )
								              . 'snapshots_destinations_panel&amp;snapshot-action=edit&amp;type=' . $classObject->name_slug . '&amp;';
								$delete_url = PSOURCESnapshot::instance()->get_setting( 'SNAPSHOT_MENU_URL' )
		...
		public static function render_destination_edit_panel() {
			...
			if ( $item ) {
				...
				?>
						<form action="<?php echo PSOURCESnapshot::instance()->get_setting( 'SNAPSHOT_MENU_URL' ); ?>snapshots_destinations_panel&amp;snapshot-action=<?php echo urlencode( sanitize_text_field( $_GET['snapshot-action'] ) ); ?>&amp;type=<?php echo urlencode( $item['type'] ); ?>" method="post">
							<?php
