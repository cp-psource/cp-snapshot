...
<?php */ ?>
		...
		*/
				...
				<table class="form-table snapshot-settings-folder-locations">
					<tr class="form-field">
						...
						<td>
							...
							<?php
								$response_object = wp_remote_get( trailingslashit( PSOURCESnapshot::instance()->get_setting( 'backupURLFull' ) ) );
		...
		private function _show_moved_server_info () {
			...
			;
			if (class_exists('PSOURCE_Dashboard') && !empty(PSOURCE_Dashboard::$site) && is_callable(array(PSOURCE_Dashboard::$site, 'allowed_user'))) {
				$url = !empty(PSOURCE_Dashboard::$ui->page_urls->support_url)
					? esc_url(add_query_arg('view', 'system', PSOURCE_Dashboard::$ui->page_urls->support_url))
			...
			<table class="form-table snapshot-settings-server-info">
				...
				<tr class="form-field">
					...
					<td>
						<?php
						$folders_array = array(
							PSOURCESnapshot::instance()->get_setting( 'backupBaseFolderFull' ),
							PSOURCESnapshot::instance()->get_setting( 'backupBackupFolderFull' ),
							PSOURCESnapshot::instance()->get_setting( 'backupRestoreFolderFull' ),
							PSOURCESnapshot::instance()->get_setting( 'backupLockFolderFull' ),
							PSOURCESnapshot::instance()->get_setting( 'backupLogFolderFull' )
						);
						sort( $folders_array );
			...
			<form action="?page=snapshots_settings_panel" method="post">
				...
				<?php
					...
					if ( ( ! isset( PSOURCESnapshot::instance()->config_data['config']['memoryLimit'] ) ) || ( empty( PSOURCESnapshot::instance()->config_data['config']['memoryLimit'] ) ) ) {
						PSOURCESnapshot::instance()->config_data['config']['memoryLimit'] = $memoryLimit;
					}
				...
				<table class="form-table snapshot-settings-segment-size">
					<tr class="form-field">
						...
						<textarea name="filesIgnore" id="filesIgnore" cols="20" rows="5"><?php
							if ( ( isset( PSOURCESnapshot::instance()->config_data['config']['filesIgnore'] ) )
							     && ( is_array( PSOURCESnapshot::instance()->config_data['config']['filesIgnore'] ) )
							     && ( count( PSOURCESnapshot::instance()->config_data['config']['filesIgnore'] ) )
							) {
								echo implode( "\n", PSOURCESnapshot::instance()->config_data['config']['filesIgnore'] );
							}
							?></textarea>
							...
							<p class="description"><?php echo sprintf( __( 'The Snapshot directory %s is automatically excluded from all snapshots',
									...
									'<code>' . trailingslashit( PSOURCESnapshot::instance()->get_setting( 'backupBaseFolderFull' ) ) . '</code>' ); ?>
						</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
					...
					foreach ( $error_reporting_errors as $error_key => $error_label ) {
						...
						<tr>
							<td>
								...
								if ( isset( PSOURCESnapshot::instance()->config_data['config']['errorReporting'][ $error_key ]['stop'] ) ) {
									echo ' checked="checked" ';
								}
								?> />
							</td>
					...
					<tr>
						...
						<td>
							...
							if ( class_exists( 'ZipArchive' ) ) {
								if ( ! isset( PSOURCESnapshot::instance()->config_data['config']['zipLibrary'] ) ) {
									PSOURCESnapshot::instance()->config_data['config']['zipLibrary'] = 'ZipArchive';
								}
								?>
								<input type="radio" name="zipLibrary" id="snapshot-settings-zip-library-ziparchive"
								       value="ZipArchive" <?php
								if ( PSOURCESnapshot::instance()->config_data['config']['zipLibrary'] == 'ZipArchive' ) {
									echo ' checked="checked" ';
								} ?> /> <?php
							...
							<?php
								...
								if ( ! isset( PSOURCESnapshot::instance()->config_data['config']['zipLibrary'] ) ) {
									PSOURCESnapshot::instance()->config_data['config']['zipLibrary'] = 'PclZip';
								}
							}
							?>
							<input type="radio" name="zipLibrary" id="snapshot-settings-zip-library-ziparchive"
...
//			echo "data_item<pre>"; print_r($data_item); echo "</pre>";
		...
		function snapshot_metaboxes_show_destination_items() {
			//echo "config_data<pre>"; print_r(PSOURCESnapshot::instance()->config_data); echo "</pre>";
			?>
			<form action="?page=snapshots_settings_panel" method="post">
			<input type="hidden" name="snapshot-action" value="settings-update"/>
