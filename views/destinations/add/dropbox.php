...
<div class="form-content">
	...
	<?php if ( isset( $item['tokens']['access']['access_token'] ) ) : ?>
		...
		<div id="wps-destination-dir" class="form-row">
			<div class="form-col-left">
				<label for="snapshot-destination-directory">
					<?php esc_html_e( 'Directory', 'cp-snapshot' ); ?> <span class="required">*</span>
				</label>
			</div>
		...
		<div id="wps-destination-auth" class="form-row">
			<div class="form-col-left">
				<label><?php esc_html_e( 'Authenticated', 'cp-snapshot' ); ?></label>
			</div>
			...
			<div class="form-col">
					...
					try {
						...
						$destination_classes = PSOURCESnapshot::instance()->get_setting( 'destinationClasses' );
						...
						$auth_error = true;
						echo '<p>', esc_html__( 'An error occurred when attempting to connect to Dropbox: ', 'cp-snapshot' ), '</p>';
					...
					<?php if ( ! $auth_error ) {
						...
						<div class="wps-auth-message wps-notice">
							<p><?php esc_html_e( "You've authenticated this Dropbox destination. To finish adding this destination, please specify a folder to store the snapshots in and click Save Destination.", 'cp-snapshot' ); ?></p>
						</div>
						<?php } else { ?>
						<div class="wps-auth-message success">
							<p><?php esc_html_e( 'This destination is authenticated and ready for use.', 'cp-snapshot' ); ?></p>
						</div>
						<?php } ?>
					<?php } ?>

				<p><small><?php esc_html_e("You can re-authenticate at any time if you wish to change this destination's details.", 'cp-snapshot'); ?></small></p>
				...
				<div class="wps-label--checkbox">
					...
					<label for="snapshot-destination-force-authorize"><?php esc_html_e( 'Force Re-Authorize with Dropbox', 'cp-snapshot' ); ?></label>
				</div>

				<div id="wps-destination-token-checkbox" class="form-row">
					<div class="form-col-left">
						<label for="snapshot-destination-token">
							<?php esc_html_e( 'Authorization code', 'cp-snapshot' ); ?> <span class="required">*</span>
						</label>
					</div>
		...
		<?php if ( isset( $account_info ) && $account_info ) : ?>
			...
			<div id="wps-destination-account" class="form-row">
				...
				<div class="form-col-left">
					<label><?php esc_html_e( 'Account', 'cp-snapshot' ); ?></label>
				</div>
					...
					<table cellpadding="0" cellspacing="0">
						<tbody>
							...
							<?php if ( !empty( $account_info->getDisplayName() ) ) : ?>
								<tr>
									<th><?php esc_html_e('Name', 'cp-snapshot'); ?></th>
								...
								<tr>
									...
									<th><?php esc_html_e('Email', 'cp-snapshot'); ?></th>
							...
							<?php if ( !empty( $account_info->getAccountId() ) ) : ?>
								<tr>
									...
									<th><?php esc_html_e('UID', 'cp-snapshot'); ?></th>
							...
							<?php if ( !empty( $account_info->getCountry() ) ) : ?>
								<tr>
									...
									<th><?php esc_html_e('Country', 'cp-snapshot'); ?></th>
									<td>
			...
			<div id="wps-destination-storage" class="form-row">
				...
				<div class="form-col-left">
					...
					<label><?php esc_html_e( 'Storage Used', 'cp-snapshot' ); ?></label>
		...
		<div id="wps-destination-auth" class="form-row">
			...
			<div class="form-col">
				...
				<div class="wps-auth-message error">
					<p><?php esc_html_e( 'This destination is using old Dropbox authentication system, please re-authenticate.', 'cp-snapshot' ); ?></p>
				</div>

				<p><small><?php esc_html_e( 'The first step in the Dropbox setup is Authorizing Snapshot to communicate with your Dropbox account. Dropbox requires that you grant Snapshot access to your account. This is required in order for Snapshot to upload files to your Dropbox account.', 'cp-snapshot' ); ?></small></p>
			...
			<div class="form-col">
				...
				<div class="wps-auth-message error">
					<p><?php esc_html_e( 'This destination is not authenticated yet.', 'cp-snapshot' ); ?></p>
				</div>
