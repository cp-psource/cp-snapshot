...
<div class="form-content">
	...
	<div id="wps-destination-type" class="form-row">
		...
		<div class="form-col">
			...
			<label><?php _e( 'Google Drive', 'cp-snapshot' ); ?></label>
		</div>
	</div>
	...
	<div id="wps-destination-dir" class="form-row">
		...
		<div class="form-col-left">
			<label for="snapshot-destination-directory"><?php _e( "Directory ID", 'cp-snapshot' ); ?> <span class="required">*</span></label>
		</div>
		...
		<div class="form-col">
			...
			<p>
				<small>
					...
					esc_html_e( "This isn't a traditional directory path like /app/snapshot/ but a unique directory ID that Google Drive use for their filesystem. ", 'cp-snapshot' );
					printf(
							__( 'To retrieve your directory ID, follow <a %s>these instructions</a>.', 'cp-snapshot' ),
			...
			<ol class="instructions" id="directory-instructions">
				<li><?php _e( 'Go to your <a href="https://drive.google.com/#my-drive" target="_blank">Drive account</a>. Navigate to or create a new directory where you want to upload the Snapshot archives. Make sure you are viewing the destination directory.', 'cp-snapshot' ); ?></li>
				<li><?php _e( 'The URL for the directory will be something similar to <em>https://drive.google.com/#folders/0B6GD66ctHXXCOWZKNDRIRGJJXS3</em>. The Directory ID would be the last part after <em>/#folders/</em>: <strong>0B6GD66ctHXXCOWZKNDRIRGJJXS3.</strong>', 'cp-snapshot' ); ?></li>
				<li><?php printf( esc_html__( 'You may specify multiple Directory IDs separated by a comma "%s"', 'cp-snapshot' ),',' ); ?></li>
			</ol>
	...
	<?php if ( $form_step > 1 ) : ?>
		...
		<div id="wps-destination-clientid" class="form-row">
			...
			<div class="form-col-left">
				<label for="snapshot-destination-clientid"><?php _e( 'Client ID', 'cp-snapshot' ); ?> <span class="required">*</span></label>
			</div>
			...
			<div class="form-col upload-progress">
				...
				<p><small><?php
					...
					printf(
						__( 'Follow <a %s>these instructions</a> to retrieve your Client ID and Secret.', 'cp-snapshot' ),
				...
				<ol class="instructions" id="clientid-instructions">
					<li><?php echo sprintf( __( 'Go to the %s', 'cp-snapshot' ), '<a href="https://console.developers.google.com/cloud-resource-manager" target="_blank">' . __( 'Google API Console', 'cp-snapshot' ) . '</a>' ) ?></li>
					<li><?php _e( 'Select an existing project or create a new one. If creating a new project, you will need to enter a name, but the ID is not important and can be ignored.', 'cp-snapshot' ); ?></li>
					...
					<li><?php _e( 'After the popup closes copy the Client ID and Client Secret from the Google page and paste into the form fields.', 'cp-snapshot' ) ?></li>
				</ol>
		...
		<div id="wps-destination-secretid" class="form-row">
			...
			<div class="form-col-left">
				<label for="snapshot-destination-clientsecret"><?php _e( 'Client Secret', 'cp-snapshot' ); ?> <span class="required">*</span></label>
			</div>
		...
		<div id="wps-destination-redirect" class="form-row">
			...
			<div class="form-col-left">
				<label for="snapshot-destination-redirecturi"><?php _e( 'Redirect URL', 'cp-snapshot' ); ?></label>
			</div>
			...
			<div class="form-col">
				...
				<div class="wps-input--copy">
					...
					<button class="button button-gray copy-to-clipboard" data-clipboard-target="#snapshot-destination-redirecturi">
						<?php esc_html_e('Copy URL', 'cp-snapshot'); ?>
					</button>
				</div>

				<p><small><?php _e( 'When your create your new credentials, add this as the redirect URL.', 'cp-snapshot' ); ?></small></p>
			</div>
	...
	<?php if ( $form_step > 2 ) : ?>
		...
		<div id="wps-destination-auth" class="form-row">
			...
			<div class="form-col-left">
				<label><?php _e( 'Authenticated', 'cp-snapshot' ); ?></label>
			</div>
			...
			<div class="form-col">
				...
				if ( $form_step > 3 && ! empty( $item_object->destination_info['access_token'] ) ) {
					...
					esc_html_e( 'This destination is authenticated and ready for use.', 'cp-snapshot' );
					echo '</p></div>';
					...
					if ( is_object( $item_object->client ) ) {
						...
						try {
							...
							echo '<p>', esc_html__( 'An error occurred authenticating with Google: ', 'cp-snapshot' ), '<br>', $e->getMessage(), '</p>';
							echo '<p>', esc_html__( 'Please check your client ID and secret ID before resubmitting this form to try again', 'cp-snapshot' ), '</p>';
							echo '</div>';
						}
						...
						if ( ! empty( $item_object->destination_info['access_token'] ) ) {
							...
							esc_html_e( 'Success. The Google Access Token has been received.', 'cp-snapshot' );
							echo ' <strong>', esc_html__( 'You must save this form one last time to retain the token.', 'cp-snapshot' ), '</strong> ';
							esc_html_e( 'The stored token will be used in the future when connecting to Google.', 'cp-snapshot' );
							echo '</p></div>';
						}
					}
					...
					echo '<div class="wps-auth-message warning"><p>';
					esc_html_e( 'To finish adding this destination you must authenticate it with Google Drive.', 'cp-snapshot' );
					echo '</p></div>';
				}
						...
						<p class="wps-auth-button">
							<a id="snapshot-destination-authorize-connection" class="button button-blue" href="<?php echo esc_url( $auth_url ); ?>">
								<?php echo empty( $item_object->destination_info['access_token'] ) ?
									esc_html__( 'Authorize', 'cp-snapshot' ) :
									esc_html__( 'Re-Authorize', 'cp-snapshot' ); ?>
							</a>
						</p>

					<?php } else {
						echo '<div class="wps-auth-message error"><p>', esc_html__( 'Unable to obtain an authorization URL from Google', 'cp-snapshot' ), '</p></div>';
					}
				}
